...
class expect_controller extends user{
	function index_action(){
		...
		if($this->config['user_enforce_identitycert']=="1"&&$resume['idcard_status']!="1"){ 
			$this->ACT_msg('index.php?c=passwd',"请先完成身份认证！"); 
		} 
	...
	function saveexpect_action(){
		...
		if($_POST['submit']){
			...
			if($eid){
				...
				$expectData['provinceid'] = $_POST['provinceid'];
				$expectData['cityid'] = $_POST['cityid'];
				...
				$expectData['three_cityid'] = $_POST['three_cityid'];
	...
	function add_action(){
		...
		if($_POST['submit']){
			...
			if($_POST['name']==""){$this->ACT_layer_msg("请填写期望岗位！",8);}
			...
			if($this->config['resume_create_exp']=='1'){
				if($_POST['workname']==""){$this->ACT_layer_msg("请填写公司名称！",8);}
				if($_POST['worksdate']==""){$this->ACT_layer_msg("请填写工作时间！",8);}
				if($_POST['worktitle']==""){$this->ACT_layer_msg("请填写公司职务！",8);}
				if($_POST['workedate']){
					if(strtotime($_POST['workedate'])<strtotime($_POST['worksdate'])){
						$this->ACT_layer_msg("工作时间不合理！",8);
					}else{
			...
			if($this->config['resume_create_edu']=='1'){
				...
				if($_POST['eduname']==""){$this->ACT_layer_msg("请填写学校名称！",8);}
				if($_POST['edusdate']==""){$this->ACT_layer_msg("请填写在校时间！",8);}
				if($_POST['eduedate']==""){$this->ACT_layer_msg("请填写离校或预计离校时间！",8);}
				if($_POST['education']==""){$this->ACT_layer_msg("请选择相关学历！",8);}
				if($_POST['eduspec']==""){$this->ACT_layer_msg("请填写相关专业！",8);}
				
				if(strtotime($_POST['eduedate'])<strtotime($_POST['edusdate'])){
					$this->ACT_layer_msg("在校时间不合理！",8);
				}else{
			...
			if($this->config['resume_create_project']=='1'){
				if($_POST['proname']==""){$this->ACT_layer_msg("请填写项目名称！",8);}
				if($_POST['prosdate']==""){$this->ACT_layer_msg("请填写项目开始时间！",8);}
				if($_POST['proedate']==""){$this->ACT_layer_msg("请填写项目结束时间！",8);}
				if($_POST['protitle']==""){$this->ACT_layer_msg("请填写项目担任职务！",8);}
				
				if(strtotime($_POST['proedate'])<strtotime($_POST['prosdate'])){
					$this->ACT_layer_msg("项目时间不合理！",8);
				}else{
	...
	function success_action(){
		if($_GET['e']){
			...
			$expect=$this->obj->DB_select_once("resume_expect","`uid`='".$this->uid."' AND `id`='".(int)$_GET['e']."'","`integrity`");
	...
	function savedescription_action(){
        ...
        $IsSuccess=$this->obj->update_once('resume',array('description'=>$_POST['description']),array('uid'=>$this->uid));
        if($IsSuccess){
            echo 1;die;
        }else{
            echo 0;die;
	        ...
	        if($_POST['table']=='description'){
	            ...
	            for($i=0;$i<count($_POST['name']);$i++){
	                ...
	                if($_POST['table']=='skill'){
	                    ...
	                    $num=6;
	                    $pic = $this->checksrc($_POST['cert'][$i]);
	                    $value="`eid`=".$eid.",`uid`=".$this->uid.",`name`='".$_POST['name'][$i]."',`pic`='".$pic."',`longtime`='".$_POST['longtime'][$i]."'";
	                    if($_POST['name'][$i]=='' && $_POST['cert'][$i]=='' && $_POST['longtime'][$i]==''){
	                        $value=1;
	                    }
