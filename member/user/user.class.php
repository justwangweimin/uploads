...
class user extends common{
	...
	function __construct($tpl,$db,$def="",$model="index",$m="") {
		...
		if($this->config['user_resume_status']=='1' && $_GET['c']!='expect' && $_GET['c']!='passwd' && $_GET['act']!='logout'){
	...
	function public_action(){
		...
		if($user['sex']==1){
			$name=mb_substr($user['name'],0,1,'utf-8').'先生';
		}elseif($user['sex']==2){
			$name=mb_substr($user['name'],0,1,'utf-8').'女士';
		}else{
			$name=$user['name'];
		}
		...
		if($_GET['c']==''){
			...
			$this->yunset("left",2);
		}elseif($_GET['c']=='commsg'||$_GET['c']=='subscribe'||$_GET['c']=='finder'||$_GET['c']=='favorite'||$_GET['c']=='look_job'||$_GET['c']=='job'||$_GET['c']=='msg'||$_GET['c']=='atn'||$_GET['c']=='partapply'||$_GET['c']=='partcollect'||$_GET['c']=='rebates'||$_GET['c']=='atnlt'||$_GET['c']=='comment'|| ($_GET['c']=='jobpack' && !$_GET['act'])){
			
			$this->yunset("left",3);
		}elseif($_GET['c']=='paylist'||$_GET['c']=='paylog'|| $_GET['c']=='integral' || $_GET['c']=='integral_reduce' || $_GET['c']=='reward_list' || $_GET['c']=='pay' || $_GET['c']=='payment'||$_GET['m']=='invitereg' || $_GET['act']=='loglist' || $_GET['act']=='withdrawlist' || $_GET['act']=='withdraw' ){
	...
	function resume($table,$url,$nexturl,$name=""){
		...
		if($_GET['del']){
			...
			$numresume=$ReusmeM->complete($resume_row); 
