...
class info_controller extends company{
	...
	function save_action(){
		...
		if($_FILES['firmpic']['tmp_name']){
		    ...
		    $picmsg = $UploadM->picmsg($firmpic,$_SERVER['HTTP_REFERER']);
		    if($picmsg['status'] == $firmpic){
		...
		if($company['email_status']==1){
		    ...
		    if($_POST['linkmail']==''){
		        $this->ACT_layer_msg("联系邮箱不能为空！",8);
		    }elseif($_POST['linkmail']&&CheckRegEmail($_POST['linkmail'])==false){
		...
		if($nid){
		    ...
		    if($company['lastupdate']==""){
		        ...
		        $IntegralM->company_invtal($this->uid,$this->config['integral_userinfo'],$auto,"首次填写基本资料",true,2,'integral',25);
		    }
		    $data['pr']=$_POST['pr'];
		    $data['mun']=$_POST['mun'];
