...
function jobadd_url(num,integral_job,type){
	var checkType = '';
...
function RefreshSelectJob(name){
	...
	if(chk_value.length==0){
		...
		var breakjob_num = $("#breakjob_num").val();
...
//批量刷新兼职
function RefreshSelectPart(name){
	var chk_value =[];
	var i=0;
	$('input[name="'+name+'"]:checked').each(function(){
		chk_value.push($(this).val());
	...
	if(chk_value.length==0){
		layer.msg("请选择要刷新的兼职！",2,8);return false;
	}else{
		$("#refreshpartids").val(chk_value);
		var partid = $("#refreshpartids").val();
		var breakpart_num = $("#breakpart_num").val();
		
		var sxpdj = $("#sxpartdj").val();
		var num = accSub(chk_value.length , breakpart_num);
		var sxpneed = accMul(sxpdj, num);
		...
		$.post(ajaxUrl, {partid:partid},function(data){
			...
			if(data.error == 1){
				...
				layer.confirm(data.msg,function(){
					layer.closeAll();
					$("#partjobid").val(chk_value);

 					$("#sxpneed").html(sxpneed);
...
function ltRefreshJob(jobId){
	...
	$.post(ajaxUrl, {jobid:jobId},function(data){
 		...
 		if(data.error == 1){
			layer.msg(data.msg, 2,9,function(){
				window.location.href='index.php?c=lt_job&s=1';
			});
		}else if(data.error == 2){
			layer.confirm(data.msg,function(){
				layer.closeAll();
