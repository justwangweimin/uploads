    ...
    function wxbind_action(){
		...
		if(((int)$_GET['usertype']=='1' || (int)$_GET['usertype']=='2') && $_SESSION['wx']['openid']){
				...
				if($usertype=="1"){
					...
					$value2['conid'] = $conid;
					if($conid['crmid']){
						$value2['crm_uid'] = $conid['crmid'];
						$value2['crm_time'] = time();
					}
				}
				...
				if($_COOKIE['regcode']!=""){
					$this->MODEL('integral')->get_integral_action($userid,"integral_invite_reg","邀请注册");
				}
				$this->MODEL('integral')->get_integral_action($userid,"integral_reg","注册赠送");
