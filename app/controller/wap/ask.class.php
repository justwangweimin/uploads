...
class ask_controller extends common{
			...
			if($authcode!=$_SESSION['authcode'] || empty($_SESSION['authcode'])){ 
			    ...
			    if($data['msg']==''){
			        ...
			        if($id){
			            ...
			            if($result==true){
			                $this->MODEL('integral')->company_invtal($this->uid,$this->config['integral_answer'],$auto,"回答问题",true,2,'integral');
			            }
	...
	function addquestions_action(){
		...
		if($cid){
			...
			if(isset($this->config['sy_day_ask_num']) && $this->config['sy_day_ask_num'] > 0) {
				...
				if($dayAskNum >= $this->config['sy_day_ask_num']){
					echo json_encode(array(
						'maxNum' => $this->config['sy_day_ask_num'],
						'askedNum' => $dayAskNum
					));
					die;
				}
			...
			if($this->usertype&&$this->uid){
				...
				if($this->usertype==2||$this->usertype==4){
					...
					$rows['nickname']=$minfo['realname'];
					$rows['pic']=$minfo['photo'];
				}
			}
			$rows['title']=$_POST['title'];
			$rows['cid']=$cid;
			$rows['content'] = str_replace(array("&amp;","background-color:#ffffff","background-color:#fff","white-space:nowrap;"),array("&",'','',''),html_entity_decode($_POST["content"],ENT_QUOTES));
			$rows['content']=strip_tags(trim($rows['content']),"<p> <br> <b>");
			$rows['uid']=$this->uid;
			$rows['add_time']=time();

			
			$rows['state'] = $this->config['ask_check'] == 1 ? 0 : 1;

			
			if($rows['state'] != 0 && $this->usertype == 2){
		...
		if($nid){
			...
			if($result==true){
				$this->MODEL('integral')->company_invtal($this->uid,$this->config['integral_question'],$auto,"发布问题",true,2,'integral');
			}
			$FriendM=$this->MODEL('friend');
			$sql['uid']=$this->uid;
			$sql['content']="发布了问答《<a href=\"".url("wap",array("c"=>"ask","a"=>"content","id"=>$nid))."\" target=\"_blank\">".$_POST['title']."</a>》。";
			$sql['ctime']=time();
			$sql['type']='2';
			$FriendM->InsertFriendState($sql);
			$M->member_log("发布了问答《".$_POST['title']."》");
			unset($_SESSION['authcode']);		
			if($this->config['ask_check'] == 1){
				echo 5;
			}
			else{
				echo 1;
	...
	function forcomment_action(){
		...
		$data['content']=str_replace(array("&amp;","background-color:#ffffff","background-color:#fff","white-space:nowrap;"),array("&",'background-color:','background-color:','white-space:'),html_entity_decode($_POST['content'],ENT_QUOTES));
