...
class index_controller extends common{
	...
	function for_link_action(){
		...
		if($this->usertype=='2'&&!empty($userid_job)&&!$this->config['com_look']){
		...
		if($row['vip_etime']>time() || $row['vip_etime']=="0"){
			...
			if($this->config['com_integral_online']=="1"){
				...
				$arr['uid']=$user['uid'];
				if($this->usertype=='2'){
					$arr['msg']="你的等级特权已经用完,将扣除".$this->config['integral_down_resume']."元，是否下载？您还可以
	...
	function RedLoginHead_action(){
		if($this->uid!=""&&$this->username!=""){
			...
			$login='<li><a href="'.$login_url.'">会员登录</a></li>';
				...
				$kjlogin = '';
				$html='<div class=" fr"><div class="yun_topLogin_cont"><div class="yun_topLogin"><a class="yun_More" href="javascript:void(0)">用户登录</a><ul class="yun_Moredown" style="display:none">'.$login.'{kjlogin}</ul></div><div class="yun_topLogin"> <a class="" href="'.Url("register").'">免费注册</a></div></div></div>';
