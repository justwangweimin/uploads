		...
		if($user_jy['uid']){
			...
			if($user['uid']){
				...
				if($user['photo']&&$user['phototype']!='1'&&(file_exists(str_replace($this->config['sy_weburl'],APP_PATH,'.'.$user['photo'])) || file_exists(str_replace($this->config['sy_weburl'],APP_PATH,$user['photo'])))){
					...
					if($user['photo']&&(file_exists(str_replace($this->config['sy_weburl'],APP_PATH,'.'.$user['photo'])) || file_exists(str_replace($this->config['sy_weburl'],APP_PATH,$user['photo'])))){
						$user['resume_photo']=str_replace("./",$this->config['sy_weburl']."/",$user['photo']);
					}else{
						if($user['sex']==1||$user['sex']=='153'){
							$user['resume_photo']=$this->config['sy_weburl'].'/'.$this->config['sy_member_icon']; 
			...
			if($this->usertype=='2'){ 
				...
				if(!empty($userid_job)&&!$this->config['com_look']){
					$user['m_status']=1;
				}
			}
			...
			if($this->uid && $this->username && ($this->usertype==2 || $this->usertype==3)){
				...
				if(is_array($row)){
					...
					if($this->usertype=='2'){
						$row=$this->DB_select_once("company_statis","`uid`='".$this->uid."'","down_resume");
					}
					if($row['down_resume']>0){
						$user['link_msg']="<a href='javascript:void(0)' onclick=\"isDownResume('$id',{$row['down_resume']},'".Url("ajax",array('c'=>'for_link'))."')\">查看联系方式</a>";
						$user['link_wapmsg']="<a href='javascript:void(0)' onclick=\"isDownResume('$id',{$row['down_resume']},'".Url("ajax",array('c'=>'forlink'))."')\">查看联系方式</a>";
					}else{
