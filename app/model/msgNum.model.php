...
class msgNum_model extends model{
	...
	function msgNum(){
		...
		$company=$this->DB_query_all("select a.uid from just_company a,just_member b where a.uid=b.uid and b.status=0;","all");
		if(count($company) > 0){
			$msgNum += count($company);
			$arr['company']=count($company);
		}
		...
		$once_job=$this->DB_select_num("once_job","`status`=0");
		if($once_job > 0){
			$msgNum += $once_job;
