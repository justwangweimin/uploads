...
class Upload{
  ...
  var $waterstring='localhost/uploads';
  var $ttf='';
  var $alpha='50';
  var $position=1;
  ...
  function Upload($paras=array()){
    if(count($paras)==false){
      $paras=array('maxsize'=>'10kb','addwatermark'=>true,'watertype'=>'txt/img','waterimg'=>'images/logo.png','waterstring'=>'','ttf'=>'','position'=>'1/2/3/4');
    }
  ...
  function movepic($src){
    $folder=$this->upfiledir.'/'.date('Ymd').'/';
    if(!file_exists($this->upfiledir)){
      @mkdir($this->upfiledir,0777,true);
    }
