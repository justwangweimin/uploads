...
function CheckTell($idcard){
    if(preg_match("/\d{3}-\d{8}|\d{4}-\d{7,8}/",$idcard)==0){
        return false;
    }
    return true;
}
...
function ArrayToString($obj,$withKey=true,$two=false){
    ...
    foreach($obj as $objkey=> $objv){
        ...
        $vtype=gettype($objv);
        if($vtype=='integer'||$vtype=='double'){
            $objstring.="$objv,";
        }else if($vtype=='string'||$vtype=='boolean'){
...
function file_get_contentsWWM($url){
    $ch=curl_init();
    $timeout=5;
...
function get_ip_city($ip){
    ...
    if(function_exists('file_get_contents')){
        ...
        $file_contents=file_get_contentsWWM($url);
    }
