...
class mysql{
  ...
  public function connect(){
    if(!$this->conn){
      $con=mysqli_connect($this->db_host,$this->db_user,$this->db_pwd,$this->db_database);
      if(!$con){
        $errorno=mysqli_connect_errno();
        if($this->show_error){
          $this->show_error("数据库不可用：",$errorno);
        }
        return;
      }
      $this->conn=$con;
  ...
  function delete($tablename,$where){
    $cachename=$tablename.$value;
    if(!$return=$this->Memcache_set($cachename)){
      $SQL="delete `".$this->def.$tablename."` WHERE $where";
      $query=$this->query($SQL);
      $return=$this->fetch_array($query);
      $this->Memcache_set($cachename,$return);
    }
  ...
  function select($tablename,$where=1,$select="*",$limit){
    $row_return=array();
    $SQL="SELECT $select FROM `".$this->def.$tablename."` WHERE $where LIMIT $limit";
    $query=$this->query($SQL);
    while($row=$this->fetch_array($query)){
      $row_return[]=$row;
    }
