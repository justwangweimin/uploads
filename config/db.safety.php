...
function common_htmlspecialchars($key,$str,$str2,$config){
  ...
  //用正则表达式进行特殊字符替换，特殊字符：\x00-\x08\x0b-\x0c\x0e-\x19==全部替换
...
if(!empty($_POST)){
  ...
  if(!isset($config['sy_safekey'])||!isset($_SESSION['xsstooken'])||$_SESSION['xsstooken']!=sha1($config['sy_safekey'])){
    foreach($_POST as $id=> $v){
      if($id!='uimage'){
        //对$v进行实体解码：单引号双引号都要解码
