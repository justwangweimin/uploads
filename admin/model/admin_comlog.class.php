...
class admin_comlog_controller extends adminCommon{
	...
	function useridmsg_action(){
		...
		$search_list[]=array("param"=>"browse","name"=>'是否查看',"value"=>array("1"=>"未查看","2"=>"已查看","3"=>"已同意","4"=>"已拒绝"));
		$ad_time=array('1'=>'今天','3'=>'最近三天','7'=>'最近七天','15'=>'最近半月','30'=>'最近一个月');
	...
	function partapply_action(){
		...
		if($_GET['keyword']){
			if($_GET['type']=="1"){ 
				...
				$job=$this->obj->DB_select_all("partjob","`name` like '%".trim($_GET['keyword'])."%'","name,id");
				$jobid=array();
				if(is_array($job)){
					foreach($job as $v){
						$jobid[]=$v['id'];
				...
				$job=$this->obj->DB_select_all("company","`name` like '%".trim($_GET['keyword'])."%'","name,uid");
				$com_id=array();
				if(is_array($job)){
					foreach($job as $v){
						$com_id[]=$v['uid'];
				...
				$where.=" and `comid` in (".@implode(",",$com_id).")";
			}
			$urlarr['type']=$_GET['type'];
			$urlarr['keyword']=$_GET['keyword'];
		}
		...
		$urlarr["page"]="{{page}}";
		$pageurl=Url($_GET['m'],$urlarr,'admin');
		$rows=$this->get_page("part_apply",$where,$pageurl,$this->config['sy_listnum']);
		if(!empty($rows)){
			foreach($rows as $v){
				$uid[]=$v['uid'];
				$pid[]=$v['jobid'];
			...
			$resume=$this->obj->DB_select_all("member","`uid` in (".@implode(",",$uid).") and `usertype`='1'","`uid`,`username`");
			$company=$this->obj->DB_select_all("company","`uid` in (".@implode(",",$cid).")","`uid`,`name`");
			$joblist=$this->obj->DB_select_all("partjob","`id` in (".@implode(",",$pid).")","`id`,`name`");
			foreach($rows as $k=>$v){
		...
		$status=array('1'=>'未查看','2'=>'已查看');
		$adtime=array('1'=>'今天','3'=>'最近三天','7'=>'最近七天','15'=>'最近半月','30'=>'最近一个月');
		$search_list[]=array("param"=>"status","name"=>'标记状态',"value"=>$status);
		$search_list[]=array("param"=>"end","name"=>'申请时间',"value"=>$adtime);
		$this->yunset("search_list",$search_list);
	...
	function delpartapply_action(){
		...
		if($_GET['del']){
			...
			$this->obj->DB_delete_all("part_apply","`id` in (".$del.")","");
			$this->layer_msg( "兼职报名(ID:".$del.")删除成功！",9,$layer_status,$_SERVER['HTTP_REFERER']);
		}else{
			$this->layer_msg( "请选择您要删除的信息！",8,1,$_SERVER['HTTP_REFERER']);
		}
	}
