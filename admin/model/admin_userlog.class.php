	...
	function index_action(){
		...
		if(trim($_GET['keyword'])){
			if($_GET['type']=="1"){
				...
				$ltinfo=$this->obj->DB_select_all("lt_info","`realname` like '%".trim($_GET['keyword'])."%'","`uid`");
				if(is_array($info)&&$info){
					foreach ($info as $v){
						$comid[]=$v['uid'];
					}
				}
				if(is_array($ltinfo)&&$ltinfo){
					foreach ($ltinfo as $v){
						$comid[]=$v['uid'];
					}
				}
	...
	function lookresume_action(){
		...
		if(trim($_GET['keyword'])){
			if($_GET['type']=="3"){ 
				...
				$ltinfo=$this->obj->DB_select_all("lt_info","`realname` like '%".trim($_GET['keyword'])."%'","realname,uid");
		...
		if(is_array($list))
		{
			...
			if($_GET['type']!="3" || !trim($_GET['keyword'])){
				...
				$ltinfo=$this->obj->DB_select_all("lt_info","`uid` in (".@implode(",",$com_ids).")","realname,uid");
			}
