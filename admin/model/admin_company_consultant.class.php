 ...
 */
class admin_company_consultant_controller extends adminCommon{
    function set_search(){
    ...
    function index_action(){
        ...
        if(!empty($rows)){
            foreach($rows as $key=> $value){
                ...
                if($value['crm_uid']!=0){
                    $crminfo=$this->obj->DB_select_once("admin_user","`uid`='".$value['crm_uid']."'","`username`,`name`");
                    $rows[$key]['crmusername']=$crminfo['username'];
                    $rows[$key]['crmname']=$crminfo['name'];
                }
            }
        ...
        $where=' `m_id` in (select id from just_admin_user_group where group_power like \'%"219"%\') and did = 0 ';
        $crmlist=$this->obj->DB_select_all("admin_user",$where);
        $this->yunset("crmlist",$crmlist);
    ...
    function save_action(){
        ...
        $gwinfo=$this->obj->DB_select_once('company_consultant','`id`='.intval($_POST['id']).'','`logo`');
        $data=array(
            ...
            'status'=>$_POST['status'],
            'weixin'=>$_POST['weixin'],
            'logo'=>$this->checksrc($_POST['logo'],$gwinfo['logo'])
        );
        ...
        if(!$_POST['id']){
            ...
            $nid=$this->obj->update_once("company_consultant",$data,array('id'=>intval($_POST['id'])));
        }
    ...
    function checkcrmid_action(){

        $value="`crm_uid`='".$_POST['cid']."'";
        $where="`id` = '".$_POST['conid']."' ";

        $nid=$this->obj->DB_update_all("company_consultant",$value,$where);

        if($nid){

            $this->obj->DB_update_all("company","`crm_uid` = '".$_POST['cid']."'","`conid` = '".$_POST['conid']."' and `crm_uid`='0' ");


            $this->ACT_layer_msg("业务员绑定成功！",9,$_SERVER['HTTP_REFERER']);
        }else{
            $this->ACT_layer_msg("业务员绑定失败失败！",8,$_SERVER['HTTP_REFERER']);
        }
    }
