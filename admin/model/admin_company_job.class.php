...
class admin_company_job_controller extends adminCommon{
	...
	function index_action(){
		...
		if($_GET['status']){
			if($_GET['status']=="1"){
				$where.=" and (`status`='1' or `state`!='1' or `edate`<".time().")";
			}else{
				$where.=" and (`status`!='1' and `state`='1' and `edate`>".time().")";
			}
	...
	function ctime_action(){
		...
		if(!empty($ids)){
			...
			$time=time()+$posttime;
			$id=$this->obj->DB_update_all("company_job","`edate`='".$time."',`state`='1'","`id` IN (".$ids.")");
