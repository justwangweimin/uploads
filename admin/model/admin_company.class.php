...
class admin_company_controller extends adminCommon{
	...
	function index_action(){
		...
		if($_GET['time']){
			...
			if($swhere){
				if($_GET['time']=='5'){
					$swhere.=" and `vip_etime`>'0' and `vip_etime`<'".time()."'";
				}else{
					$swhere.=" and `vip_etime`>'".time()."' and `vip_etime`<'".strtotime($num)."'";
				}
			}else{
				if($_GET['time']=='5'){
					$swhere.=" `vip_etime`>'0' and `vip_etime`<'".time()."'";
				}else{
	...
	function comeditsave_action(){
	    ...
	    if(!CheckRegUser($_POST['username'])){
	    ...
	    if(is_array($user)){
	        ...
	        if($_POST['status']=='2'){
	            if($mem['status']!='2'){
	                ...
	                $retval = $notice->sendEmailType(array("email"=>$mem['email'],"lock_info"=>$_POST['lock_info'],"uid"=>$data['uid'],"name"=>$data['name'],"type"=>"lock"));
	                if($retval['status'] == -1){
	        ...
	        $content=str_replace(array("&amp;","background-color:#ffffff","background-color:#fff","white-space:nowrap;"),array("&",'','',''),html_entity_decode($_POST['content'],ENT_QUOTES));
	        ...
	        if($ratingid != $statis['rating']){
	            ...
	            $value3.="`zph_num`='".$_POST['zph_num']."',";
 	            $value3.="`lt_job_num`='".$_POST['lt_job_num']."',";
	            $value3.="`lt_down_resume`='".$_POST['lt_down_resume']."',";
	            $value3.="`lt_editjob_num`='".$_POST['lt_editjob_num']."',";
	            $value3.="`lt_breakjob_num`='".$_POST['lt_breakjob_num']."'";
			...
			if($username==""||strlen($username)<2||strlen($username)>15){
				...
				if(is_array($user)){
					...
					if($uid>0){
						...
						$this->obj->DB_insert_once("company","`uid`='$uid',`name`='$name',`shortname`='$shortname',`linkphone`='$linkphone',`linktel`='$moblie',`linkmail`='$email',`address`='$address'");
						$rat_arr = @explode("+",$rating_name);
						$value = "`uid`='$uid',";
	...
	function recommend_action(){
		...
		$this->model('log')->admin_log("知名企业(ID:".$_GET['id'].")设置成功");
		echo $nid?1:0;die;
	}
	function del_action(){
		$this->check_token();
	    if($_GET['del']){
	    	$del=$_GET['del'];
	    	if($del){
				$del_array=array("member","company","company_job","company_cert","company_news","company_order","company_product","company_show","banner","company_statis","question","attention","lt_job","hotjob","invoice_record","special_com","zhaopinhui_com","partjob","answer","answer_review","evaluate_log","subscribe","subscriberecord","friend_state");
	...
	function checksitedid_action(){
		if($_POST['uid']){
			...
			if($uid){
				...
				$Table = array('member','company','company_statis','company_job','company_cert','company_news','company_order','company_product','invoice_record','partjob','hotjob');
	...
	function saveusername_action(){ 
		if($_POST['username']){
			if(CheckRegUser($_POST['username'])==false){
					echo 2;die;
			}
			$username=$_POST['username'];
	...
	function mjobshow_action(){
	    ...
	    if($_POST['update']){
	        ...
	        $_POST['description'] = str_replace("&amp;","&",html_entity_decode($_POST['content'],ENT_QUOTES));
          ...
          $notice = $this->MODEL('notice');
	        ...
	        if($data!=""){
	            foreach($data as $key=>$val){
                  $notice->sendEmailType($val);
                  $notice->sendSMSType($val);
	            }
	        }
	        $aid = @implode(",",$idlist);
	...
	function morderadd_action(){
	    ...
	    if(isset($_POST['insert'])){
	        ...
	        if($_POST['fs']==1){ 
	            ...
	            $statis=$this->obj->DB_select_once('company_statis',"`uid`='".$comid."'","pay,integral");
	...
	function mshowup_action(){
	    if($_POST['submitbtn']){
	        ...
	        $nid=$this->obj->DB_update_all("company_show","`title`='".$_POST['title']."',`sort`='".$_POST['showsort']."',`ctime`='".$time."'","`uid`='".(int)$_POST['uid']."'and `id`='".$_POST['id']."'");
	        if($nid){
