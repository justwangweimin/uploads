...
class admin_tpl_controller extends adminCommon{
	...
	function comptplsave_action(){
	    if($_POST['pic']=="") {
	        ...
	        $row = $this->obj->DB_select_once('company_tpl','`id`="'.(int)$_POST['id'].'"');
	        $pic = $this->checksrc($_POST['pic'],$row['pic']);
	        $UploadM = $this->MODEL('upload');
	        $upload=$UploadM->Upload_pic('','','','../data/upload/company'.'/'.date('Ymd').'/');
	        $thumb=$upload->makeThumb('../'.substr($pic, strpos($pic,'data/upload/')),350,350,'_S_');
	        unlink_pic('.'.$pic);
	    }
		...
		$data['url'] = $_POST['url'];
		$data['pic'] = str_replace('../', '', $thumb);
		$data['status'] = $_POST['status'];
		$data['price'] = $_POST['price'];
		$data['service_uid'] = $_POST['service_uid'];
		if($_POST['id']){
		    $id=$this->obj->update_once("company_tpl",$data,array("id"=>$_POST['id']));
			$msg="企业模板(ID:".$_POST['id'].")更新";
		}else{
		    $id=$this->obj->insert_into("company_tpl",$data);
			$msg="企业模板(ID:".$id.")添加";
		}
	...
	function resumetplsave_action(){
		if($_POST['pic']=="") {
		    ...
		    $row = $this->obj->DB_select_once('resumetpl','`id`="'.(int)$_POST['id'].'"');
