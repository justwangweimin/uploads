...
class admin_partjob_controller extends adminCommon{
	...
	function index_action(){
		...
		if(is_array($rows)){
			...
			$joblist=$this->obj->DB_select_all("part_apply","`jobid` in (".@implode(",",$jid).") GROUP BY jobid","`jobid`,COUNT(jobid) as jnum");
			include PLUS_PATH."part.cache.php";
			foreach($rows as $k=>$v){
	...
	function refresh_action() 
	{
		...
		$this->model('log')->admin_log("兼职职位(ID".$_POST['ids']."刷新成功");
	}
}
?>