...
class reward_list_controller extends adminCommon{
		...
		if(intval($_POST['id'])){
			...
			if($status>0&&$change['status']=='0'){
				if($status=='1'){
					...
					$IntegralM->company_invtal($change['uid'],$change['integral'],true,"未通过积分兑换",true,2,'integral',24); 
	...
	function statuss_action(){
		...
		$this->model('log')->admin_log("批量审核(ID:".$_POST['allid'].")审核成功");
		    ...
		    $rowss=$this->obj->DB_select_all("change","`id` in(".$del.")","`uid`,`gid`,`num`,`integral`,`usertype`,`status`");
				if($rowss&&is_array($rowss)){
					foreach($rowss as $val){
						if($val['status']==0){
							$IntegralM=$this->MODEL('integral');
							$IntegralM->company_invtal($val['uid'],$val['integral'],true,"取消兑换",true,2,'integral',24);
