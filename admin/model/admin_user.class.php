	...
	function deluser_action()
	{
		...
		if(isset($_GET['uid'])){
			...
			if($result){
				...
				$this->obj->DB_delete_all('crmnew_concern', "uid = '{$uid}'");
				$this->obj->DB_delete_all('crmnew_return_visit', "uid = '{$uid}'");
				$this->obj->DB_update_all('company', 'crm_uid = 0, crm_status = 0', "crm_uid = '{$uid}'");
			}
