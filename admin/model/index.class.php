...
class index_controller extends adminCommon{
    function index_action($set=""){
        ...
        $menurows=$this->obj->DB_select_all("admin_navigation","`display`<>1 and `id` in(".@implode(',',$power).") order by `sort` asc");
        $i=0;
        $j=0;
        $a=0;
        $b=0;
    ...
    function del_cache_action(){
        ...
        if($config==false){
            $this->obj->DB_insert_once("admin_config","`name`='cachecode',`config`='".$cachecode."'");
        }else{
            $this->obj->DB_update_all("admin_config","`config`='".$cachecode."'","`name`='cachecode'");
        }
        $this->web_config();
