...
class config_controller extends adminCommon{
    function index_action(){
        if(strpos($this->config['sy_weburl'],'https')!==false){
            $this->config['mapurl']='https://api.map.baidu.com/api?v=3.0&ak='.$this->config['map_key'].'&s=1';
        }else{
            $this->config['mapurl']='http://api.map.baidu.com/api?v=3.0&ak='.$this->config['map_key'];
        }
    ...
    function save_action(){
        if($_POST['config']){
            ...
            if($_POST['map_key']){
                if(strpos($this->config['sy_weburl'],'https')!==false){
                    $_POST['mapurl']='https://api.map.baidu.com/api?v=3.0&ak='.$_POST['map_key'].'&s=1';
                }else{
                    $_POST['mapurl']='http://api.map.baidu.com/api?v=3.0&ak='.$_POST['map_key'];
                }
            }
